    ...
    return {
        ...
        initDatatable: function (jsonObj) {
            ...
            var datatableCatalogPage = function () {
                ...
                $("#kt_catalog_emulator_reload").off("click").on("click", function () {
                    var $btn = $(this).prop('disabled', true).html('<i class="la la-refresh la-spin"></i> Recarregando...');
                    $.ajax({
                        url: '/housekeeping/api/catalog/reloadcatalog',
                        type: 'post',
                        headers: { 'Authorization': 'housekeeping' },
                        dataType: 'json',
                        success: function (data) {
                            if (data.status === 'success') {
                                toastr.success('Catálogo recarregado! Os jogadores foram notificados.');
                            } else {
                                toastr.error('Erro: ' + data.message);
                            }
                        },
                        error: function () {
                            toastr.error('Falha ao comunicar com o emulador.');
                        },
                        complete: function () {
                            $btn.prop('disabled', false).html('<i class="la la-refresh"></i> Recarregar Catálogo');
                        }
                    });
                });
            };
