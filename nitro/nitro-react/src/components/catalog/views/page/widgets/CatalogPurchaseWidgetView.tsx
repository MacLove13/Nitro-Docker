...
import { Button, Flex, LayoutLoadingSpinnerView } from '../../../../../common';
...
export const CatalogPurchaseWidgetView: FC<CatalogPurchaseWidgetViewProps> = props =>
{
    ...
    return (
        <Flex gap={ 2 } className="nitro-catalog-purchase-actions">
            { (!noGiftOption && !currentOffer.isRentOffer) &&
                <Button className="nitro-catalog-gift-button" disabled={ ((purchaseOptions.quantity > 1) || !currentOffer.giftable || isLimitedSoldOut || (purchaseOptions.extraParamRequired && (!purchaseOptions.extraData || !purchaseOptions.extraData.length))) } onClick={ event => purchase(true) }>
