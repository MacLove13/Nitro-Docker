...
import { CatalogCrackablePrizesView } from '../../../../catalog/views/page/layout/CatalogCrackablePrizesView';
...
export const InfoStandWidgetFurniView: FC<InfoStandWidgetFurniViewProps> = props =>
{
    ...
    const [ crackableTypeId, setCrackableTypeId ] = useState(0);
    ...
    useEffect(() =>
    {
        ...
        setShowCrackablePrizes(false);

        // Extract base typeId (items_base.id) for crackable prizes lookup
        if(isCrackable) {
            const roomObj = GetRoomEngine().getRoomObject(roomSession.roomId, avatarInfo.id, avatarInfo.category);
            const tid = roomObj ? roomObj.model.getValue<number>(RoomObjectVariable.FURNITURE_TYPE_ID) : 0;
            setCrackableTypeId(tid ?? 0);
        } else {
            setCrackableTypeId(0);
        }
        ...
        <Column gap={ 1 } alignItems="end">
            { showCrackablePrizes &&
                <CatalogCrackablePrizesView
                    spriteId={ crackableTypeId }
                    itemName={ avatarInfo.name }
            ...
            <Column className="nitro-infostand rounded">
                <Column overflow="visible" className="container-fluid content-area" gap={ 1 }>
                    ...
                    <Column gap={ 1 }>
                        ...
                        { (isCrackable && crackableTypeId > 0) &&
                            <Flex>
                                <Text variant="white" small underline pointer onClick={ () => setShowCrackablePrizes(true) }>
                                    Ver premios & probabilidades
                                </Text>
                            </Flex> }
                    </Column>
                    { (isJukeBox || isSongDisk) &&
