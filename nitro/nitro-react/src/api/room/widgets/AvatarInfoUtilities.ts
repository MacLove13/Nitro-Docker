...
export class AvatarInfoUtilities
{
    ...
    public static getUserInfo(category: number, userData: IRoomUserData): AvatarInfoUser
    {
        ...
        userInfo.health = Math.max(0, Math.min(100, ((userData as IRoomUserData & { health?: number }).health ?? 100)));
        userInfo.stamina = Math.max(0, Math.min(100, ((userData as IRoomUserData & { stamina?: number }).stamina ?? 100)));
