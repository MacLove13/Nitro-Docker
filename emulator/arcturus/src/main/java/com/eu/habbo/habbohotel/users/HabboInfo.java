...
public class HabboInfo implements Runnable {
    ...
    public HabboInfo(ResultSet set) {
        try {
            ...
            this.health = this.clampStat(set.getInt("health"));
            this.stamina = this.clampStat(set.getInt("stamina"));
    ...
    public void setHealth(int health) {
        this.health = this.clampStat(health);
        this.run();
    }
    ...
    public void setStamina(int stamina) {
        this.stamina = this.clampStat(stamina);
        this.run();
    }
    ...
    public void run() {
        ...
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection(); PreparedStatement statement = connection.prepareStatement("UPDATE users SET motto = ?, online = ?, look = ?, gender = ?, credits = ?, health = ?, stamina = ?, last_login = ?, last_online = ?, home_room = ?, ip_current = ?, `rank` = ?, machine_id = ?, username = ? WHERE id = ?")) {
            ...
            statement.setInt(6, this.health);
            statement.setInt(7, this.stamina);
            statement.setInt(8, Emulator.getIntUnixTimestamp());
            statement.setInt(9, this.lastOnline);
            statement.setInt(10, this.homeRoom);
            statement.setString(11, this.ipLogin);
            statement.setInt(12, this.rank != null ? this.rank.getId() : 1);
            statement.setString(13, this.machineID);
            statement.setString(14, this.username);
